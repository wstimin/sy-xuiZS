...
export const AccountView: React.FC<AccountViewProps> = ({ account, loading, onRefresh }) => {
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
      ...
      <section className="space-y-3">
        ...
        <div className="overflow-x-auto border border-white/10 rounded-lg">
          <table className="w-full text-sm min-w-[650px]"><thead className="bg-white/5 text-zinc-400"><tr><th className="text-left p-3">订单号</th><th className="text-left p-3">套餐</th><th className="text-left p-3">金额</th><th className="text-left p-3">状态</th><th className="text-left p-3">创建时间</th></tr></thead>
            <tbody>{account?.orders.map(order => { const snapshot = JSON.parse(order.planSnapshot || '{}'); return <tr key={order.id} className="border-t border-white/5"><td className="p-3 font-mono text-xs">{order.orderNo}</td><td className="p-3">{snapshot.name || '-'}</td><td className="p-3">{formatMoney(order.amountCents)}</td><td className="p-3">{orderLabels[order.status] || order.status}</td><td className="p-3 text-zinc-500">{formatDate(order.createdAt)}</td></tr>; })}</tbody>
          </table>
